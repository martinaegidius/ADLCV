...
class UNet(nn.Module):
    def __init__(self, img_size=16, c_in=3, c_out=3, time_dim=256, device="cpu", channels=32, num_classes=None):
        ...
        if num_classes is not None:
            ...
            self.label_emb = nn.Sequential(nn.Linear(num_classes,time_dim),nn.GELU(),nn.Linear(time_dim,time_dim))
    ...
    def forward(self, x, t, y=None):
        ...
        if y is not None:
            ...
            ##the following two steps are part of ddpm_train.py
            #y = F.one_hot(y,num_classes=self.num_classes)
            #y = y.to(torch.FloatTensor)
            
            y = self.label_emb(y)
            t += y #I don't know why, but I guess this is what we want, TA says it's just a hack :-) 
...
class Classifier(nn.Module):
    def __init__(self, img_size=16, c_in=3, labels=5, time_dim=256, device="cuda", channels=32):
        ...
        #self.lin1 = nn.Linear(c_in*img_size**2,time_dim*2)
        #self.lin2 = nn.Linear(time_dim*2,time_dim)
        ...
        self.Decoder = nn.Linear(128*2*2,labels)
    def forward(self, x, t):
        #if(y is not None):
        #    y = F.one_hot(y,num_classes = self.num_classes).to(dtype=torch.float32)
        #    enc = self.Encoder(x,t,y)
        #    pred = self.Decoder(torch.flatten(enc,start_dim=1))
        #else:
        t = t.unsqueeze(-1).type(torch.float)
        t = pos_encoding(t, self.time_dim, self.device)
        ...
        x4 = self.bot2(x4)
        x4 = self.bot3(x4)
        x4 = torch.flatten(x4,start_dim=1,end_dim=-1)
        pred = self.Decoder(x4)
        return pred