...
class Diffusion:
    ...
    def classifier_conditioning(self, x_t, t, y, mean, std, classifier_scale=1):
        ...
        with torch.enable_grad():
            ...
            new_mu = mean + classifier_scale * std * gradient.float()

        return new_mu


    def p_mean_std(self, model, x_t, t, y=None,classifier_scale=1):
        """
        ...
        if y is not None and self.diff_type=='DDPM-cg':
            mean = self.classifier_conditioning(x_t, t, y, mean, std,classifier_scale)      
        return mean, std


    def p_sample(self, model, x_t, t, y=None,classifier_scale=1):
        """
        Sample from p(x{t-1} | x_t) using the reverse process and model
        """
        mean, std = self.p_mean_std(model, x_t, t, y,classifier_scale=1)
        if t[0] > 1:
    ...
    def p_sample_loop(self, model, batch_size, timesteps_to_save=None, y=None, verbose=True,classifier_scale=1):
        """
        y is class label
        """
        if verbose:
        ...
        with torch.no_grad():
            ...
            for i in pbar:
                ...
                # T-1, T-2, .... 0
                x = self.p_sample(model, x, t, y,classifier_scale=classifier_scale)
