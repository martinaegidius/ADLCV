...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        keys = rearrange(keys, 'batch_sz seq_len (n_heads head_dim) -> batch_sz seq_len n_heads head_dim',n_heads=self.num_heads,head_dim=self.head_dim)
        # Secondly merge the batch_size with the num_heads
        keys = rearrange(keys,'batch_sz seq_len n_heads head_dim -> (batch_sz n_heads) seq_len head_dim')
        #rearrange(queries, 'batch_sz seq_len embed_dim -> batch_sz seq_len (n_heads head_dim)',n_heads=self.num_heads,head_dim=self.head_dim)
        #rearrange(queries,'batch_sz seq_len n_heads head_dim -> (batch_sz n_heads) seq_len head_dim')
        
        #rearrange(values, 'batch_sz seq_len embed_dim -> batch_sz seq_len (n_heads head_dim)',n_heads=self.num_heads,head_dim=self.head_dim)
        #rearrange(values,'batch_sz seq_len n_heads head_dim -> (batch_sz n_heads) seq_len head_dim')
        
        # HINT repeat the same process for queries and values
        queries = rearrange(queries, 'batch_sz seq_len (n_heads head_dim) -> batch_sz seq_len n_heads head_dim',n_heads=self.num_heads,head_dim=self.head_dim)
        queries = rearrange(queries,'batch_sz seq_len n_heads head_dim -> (batch_sz n_heads) seq_len head_dim')
        
        values = rearrange(values, 'batch_sz seq_len (n_heads head_dim) -> batch_sz seq_len n_heads head_dim',n_heads=self.num_heads,head_dim=self.head_dim)
        values = rearrange(values,'batch_sz seq_len n_heads head_dim -> (batch_sz n_heads) seq_len head_dim')
        

        # Compute attetion logits
        attention_logits = torch.bmm(queries,torch.transpose(keys,1,-1)) # multiply queries and keys
        attention_logits = attention_logits * self.scale
        attention = self.softMax(attention_logits) # softmax on attention
...
class TransformerClassifier(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers, max_seq_len,
        ...
        if self.pool == 'cls':
            self.cls_token = nn.Parameter(1,1,embed_dim,requires_grad=True) # Institate an nn.Parameter with size: 1,1,embed_dim
            max_seq_len +=1
