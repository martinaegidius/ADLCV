...
class Diffusion:
    def __init__(self, T=500, beta_start=1e-4, beta_end=0.02, img_size=16, variance_schedule="linear",device="cuda"):
        """
        ...
        if(variance_schedule=="linear"):
            self.betas = self.get_betas(variance_schedule).to(device)
            self.alphas = 1. - self.betas
            self.alphas_bar = torch.cumprod(self.alphas, dim=0) # cumulative products of alpha
        elif(variance_schedule=="cosine"):
            self.alphas_bar, self.betas = self.get_betas(variance_schedule)
    ...
    def get_betas(self, schedule='linear'):
        ...
        # add your own (e.g. cosine)
            ...
            t = torch.linspace(0,self.T-1,self.T)
            t0 = torch.zeros_like(t)
            alphas_cos = f(t)/f(t0)
            return alphas_cos, torch.clamp(1-alphas_cos/torch.cat((torch.Tensor([1]),alphas_cos[:alphas_cos.shape[0]-1])),min=None,max=0.999) #need check correctness later
        else :
            raise NotImplementedError('Not implemented!')
    
    def eye_like(self, x):
        #obsolete: made for sampling N(0,I) but this is not what we want
        batch_sz = x.shape[0]
        channels = x.shape[1]
        stds = torch.eye(x.shape[2],x.shape[-1]) 
        stds = stds.repeat(batch_sz,channels,1,1)
        return stds.to(self.device)
