...
def train(device='cpu', T=500, img_size=16, input_channels=3, channels=32, time_dim=256,
    ...
    diffusion = Diffusion(img_size=img_size, T=T, beta_start=1e-4, beta_end=0.02, variance_schedule="cosine",device=device)

    optimizer = optim.AdamW(model.parameters(), lr=lr)
    mse = torch.nn.MSELoss(reduction="mean") # use MSE loss 
